...
let placeholderComments: Comment[] = [
    {
        id: 'comment-1',
        taskId: 'task-1',
        userId: '1',
...
export async function getProjectById(id: string): Promise<Project | undefined> {
    return Promise.resolve(placeholderProjects.find(p => p.id === id));
}
...
export async function createProject(projectData: Omit<Project, 'id'>): Promise<Project> {
    ...
    placeholderProjects.unshift(newProject);
    return Promise.resolve(newProject);
}

// TASKS
export async function getTasks(): Promise<Task[]> {
    return Promise.resolve(placeholderTasks);
}

export async function getTaskById(id: string): Promise<Task | undefined> {
    return Promise.resolve(placeholderTasks.find(t => t.id === id));
}
...
export async function updateTask(taskId: string, taskData: Partial<Task>): Promise<Task | undefined> {
    const taskIndex = placeholderTasks.findIndex(t => t.id === taskId);
    if (taskIndex === -1) {
        return Promise.resolve(undefined);
    }
    const updatedTask = { ...placeholderTasks[taskIndex], ...taskData };
...
export async function getCommentsByTaskId(taskId: string): Promise<Comment[]> {
    return Promise.resolve(placeholderComments.filter(c => c.taskId === taskId));
}
