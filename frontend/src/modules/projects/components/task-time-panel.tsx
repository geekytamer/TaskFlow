...
export function TaskTimePanel({ taskId }: { taskId: string }) {
  ...
  const submit = async () => {
    ...
    if (!Number.isFinite(value) || value <= 0) {
      toast({ variant: 'destructive', title: 'Enter hours greater than zero.' });
      return;
    }
    setSaving(true);
    try {
      ...
      toast({ variant: 'destructive', title: 'Could not log time', description: error instanceof Error ? error.message : undefined });
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (id: string) => {
    try {
      ...
      toast({ variant: 'destructive', title: 'Could not delete entry', description: error instanceof Error ? error.message : undefined });
    }
  };
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        {totalMinutes > 0 ? (
          <span className="text-sm font-medium">{formatDuration(totalMinutes)} logged</span>
        ) : null}
      </div>
      ...
      <div className="grid grid-cols-[80px_1fr_auto] items-end gap-2">
        ...
        <div className="space-y-1">
          ...
          <Input className="h-8" value={note} onChange={(e) => setNote(e.target.value)} placeholder="What did you work on?" />
        </div>
      ...
      {entries.length > 0 ? (
        ...
        <p className="mt-3 text-xs text-muted-foreground">No time logged yet.</p>
      )}
    </div>
  );
}
